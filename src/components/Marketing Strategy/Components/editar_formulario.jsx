...
export function EditarEstrategia() {
    ...
    async function fetchData() {
      ...
      try {
        const response = await fetch(
          `/api/MarketingStrategy/obtenerFormulario?id=${id}`
        );
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await fetch(
        `/api/MarketingStrategy/actualizarFormulario?id=${id}`,
        {
          method: "POST",
          headers: {
    ...
    try {
      const response2 = await fetch("/api/Reminder/EnvioEvento", {
        ...
        body: JSON.stringify({
          formData2: {
            ...
            descripcion: `<strong>${nombre} ${apellidos}</strong> ha editado la estrategia con el siguiente id: <strong>${id}</strong>`,
            id: idUser,
            dpto: departamento,
          },
        }),
      ...
      if (response2.ok) {
        ...
        Swal.fire(
          "Error",
          "Error al enviar la alerta de edición de estrategia",
          "error"
        );
      }
    } catch (error) {
  ...
  return (
    <Card style={{ maxWidth: "100rem" }} className="w-full mx-auto">
      <CardHeader>
        ...
        <CardDescription>
          Actualice los detalles del evento, costos, especificaciones, productos
